...
```{r}
#library(dplyr)
#class(slowdown_dataset$CPUS)
#slowdown_dataset$p1 <- 100*log(as.numeric(slowdown_dataset$CPUS))
#slowdown_dataset$wp <- 100*log(as.numeric(slowdown_dataset$OIL))

#install.packages("lpirfs", dependencies = T)
#install.packages("data.table")
#library(lpirfs)
 #slowdown_dataset %>% select(Dates, OIL, CPUS, YUS, SUS) %>% lp_lin_panel(., data_sample = FULL, shock=OIL, c_exog_data=OIL)
...
library(vars)
# putting my data in a time series object
ts_us <- slowdown_dataset %>% select(OIL, YUS) %>% ts(., start=c(1970,1), frequency = 4)
...
# WE first want to create seperate objects for each variable 

ts_us <- slowdown_dataset %>% select(OIL, YUS, SUS, CPUS) %>% ts(., start=c(1970,1), frequency = 4)
plot(ts_us, main = "") # This is done before as well. No change. 

# Prove that one lag is optimal. 
# We want to get a reducef-form VAR to get an appropriate object that is manipulated into the structural form model 

var1 <- VAR(ts_us, p=1, type="both")
summary(var1)
...
irf1 <- irf(model1, impulse="OIL", response = "SUS", boot=FALSE, n.ahead =40)
...
plot(irf(svar.one, impulse = "OIL", response = "OIL", n.ahead=40, boot=TRUE))
